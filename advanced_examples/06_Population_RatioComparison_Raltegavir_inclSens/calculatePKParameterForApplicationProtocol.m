...
[intervalList,~,ix_PK] = unique({PKParameterTemplate.timeRange},'stable');

intervalList = setdiff(intervalList,'ratio','stable');
...
% fin ratio PK Parameter
jj_R_lin = strcmp({PKParameterTemplate.name},'R_lin');
jj_AUCinf_int1 = strcmp({PKParameterTemplate.name},'AUC_inf_t1');
jj_AUClast_intlast = strcmp({PKParameterTemplate.name},'AUC_tLast_minus_1_tLast');

if any(jj_R_lin)
    PKParameterTemplate(jj_R_lin).value = PKParameterTemplate(jj_AUClast_intlast).value./PKParameterTemplate(jj_AUCinf_int1).value;
end
...
switch flag
    ...
    case 'multiDose'
        ...
        % Ratios of auc has to be at the end, other wise we have problems
        % with setdiff in intervall selection
        PKParameterTemplate(end+1) = iniPKParameterTemplate('R_lin','R_{lin}','','1','R_lin','ratio');
