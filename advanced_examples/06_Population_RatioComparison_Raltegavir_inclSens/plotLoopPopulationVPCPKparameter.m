...
for iSet = 1:length(PopRunSet)
    ...
    if isempty(o)
        o = getPKperSet(PKPList,OutputList,PKParameterTemplate,iSet,length(PopRunSet),flag);
    else
        ...
        for iO = 1:length(PKPList)
            ...
            if ~isempty(PKPList{iO})
                % get corresponding index in o
                jj = strcmp({o.pathID},OutputList(iO).pathID);
                if any(jj)
                    iOo = find(jj);
                else
                    otmp = getPKperSet(PKPList,OutputList,PKParameterTemplate,iSet,length(PopRunSet),flag);                    
                    jj = strcmp({otmp.pathID},OutputList(iO).pathID);
                    o(end+1) = otmp(jj); %#ok<AGROW>
                    iOo = length(o);
                end
            
            
                % get indices
                [jj,ixTarget] = ismember(OutputList(iO).pKParameterList(1,:),o(iOo).pKParameterList(1,:));
                ixSource = [find(jj) find(~jj)];
                ixTarget = ixTarget(jj);
                
                % add new values to structure
                for iNew = find(~jj)
                    ixTarget(end+1) =  size(o(iOo).pKParameterList,2)+1;%#ok<AGROW>
                    o(iOo).pKParameterList(1:3,ixTarget(end)) = OutputList(iO).pKParameterList(:,iNew);%#ok<AGROW>
                    
                    % add displayname
                    [~,ix] = ismember(o(iOo).pKParameterList(1,ixTarget(end)),{PKParameterTemplate.name});
                    o(iOo).pKParameterList{4,ixTarget(end)} = PKParameterTemplate(ix).reportName; %#ok<AGROW>                    
                end
                
                % add values
                nInd = length([PKPList{iO}(:,1).value]);
                nIndOld = size(o(iOo).values,1);
                if strcmp(flag,'matrix')
                        if nInd > nIndOld
                            o(iOo).values(nIndOld+1:nInd,:,:) = nan; %#ok<AGROW>
                        end
                ...
                for k = 1:length(ixTarget)
                    tmp(1:nInd) = [PKPList{iO}(:,ixSource(k)).value]...
                        .*OutputList(iO).pKParameterList{3,ixSource(k)};
                    switch flag
                        case 'matrix'                            
                            o(iOo).values(:,iSet,ixTarget(k)) = tmp; %#ok<AGROW>
                        case 'vector'
                            o(iOo).values(nIndOld + [1:nInd],ixTarget(k)) = tmp; %#ok<NBRAK,AGROW>
...
return
 ...
 function  o = getPKperSet(PKPList,OutputList,PKParameterTemplate,iSet,nSet,flag)
    
 for iO = 1:length(PKPList)
         
     o(iO).reportName = OutputList(iO).reportName; %#ok<AGROW>
     o(iO).pathID = OutputList(iO).pathID; %#ok<AGROW>
     ...
     if ~isempty( OutputList(iO).pKParameterList)
         ...
         switch flag
             case 'matrix'
                 o(iO).values = nan(length([PKPList{iO}(:,1).value]),nSet,length(PKPList{iO}));
